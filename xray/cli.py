"""
Code X-Ray CLI — core logic.

Used by:
- xray.py (git clone / direct run)
- xray.__main__ (python -m xray)
- pip entry_points (xray / code-xray commands)
"""
...
def _template_path() -> str:
    ...
    candidates = [
        os.path.join(pkg_dir, "templates", "dashboard.html"),   # pip install / package data
        os.path.join(repo_root, "templates", "dashboard.html"), # git clone (root templates/)
    ]
    for c in candidates:
        if os.path.isfile(c):
...
def main(argv=None) -> int:
    ...
    if rc:
        _log("[xray] Loaded .xrayrc config")
